...
<!-- ```{r,echo=FALSE}
cat('squid_data <- simulate_population(
  n=2000,
  response_name =, "body_mass",
  parameters = list(',
colorize('    intercept=10,',"red"),
'    observation = list(
...
$$
 ...
 . Note that the order of the names and betas has to match. We can then specify the residual variance, here as 1 (but can be anything)
```r
    residual = list(
          vcov = 1
        )
...
Its worth noting that these values are not *exactly* what we simulated. That is to be expect - simulation involves *randomly* generating data, which means that here will be stochasticity in the simulated sample, and in our estimates of the underlying parameters.
<!-- 

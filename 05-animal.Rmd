...
squid_data <- simulate_population(
  parameters = list(
    ...
    animal= list(
      ...
      vcov =matrix(c(0.1,-0.1,-0.1,0.4), nrow=2, ncol=2 )
      ),
    residual = list(
      names="residual",
      vcov = 0.1
...
library(nadiv)
D <- makeD(BTped,invertD = FALSE, returnA = FALSE, det=FALSE)$D

ds <- data.frame(animal=BTped[,"animal"],animal2=BTped[,"animal"])
